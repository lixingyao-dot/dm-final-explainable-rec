...
from src.ncf_models.ncf_bpr import NCFBPR, train_ncf_bpr
from src.evaluate import evaluate_model_sampled, print_metrics


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--models", nargs="+", default=["ncf"],
                        choices=["ncf", "ncfbpr", "all"])
    parser.add_argument("--train", action="store_true")
    args = parser.parse_args()

    run_models = set(args.models)
    if "all" in run_models:
        run_models = {"ncf", "ncfbpr"}
