"""NCF + Bias + BPR loss — all improvements combined.

- BPR pairwise loss (ranking objective)
- user_bias + item_bias + global_bias (popularity signal)
- embedding_dim=32 (fewer params for small data)
- neg_ratio=9 (more negatives for richer training signal)
"""
...
class NCFBiasBPR(nn.Module):
    """NCF with bias terms, trained with BPR loss."""
...
def train_ncf_bpr_bias(model, train_pos_df, val_df, config, n_items, device=None):
    if device is None:
    ...
    epoch_iter = tqdm(range(config["model"]["epochs"]), desc="NCFBias+BPR Training", unit="epoch")
    ...
    plot_training_history(
        ...
        "outputs/plots/ncf_bpr_bias_training.png",
